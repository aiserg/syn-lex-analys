...
(function() {
  var ConvRes, Convol, Input, InputMassive, Keywords, Literals, Output, Separators, deleteConvol, doBrackets, doPriority, doUnar, isKeyword, isLiteral, isNumber, isSeparator, lexicalAnalysis, streamWords, syntaxAnalys, turn, value_command,
  ...
  isLiteral = function(str, command) {
    ...
    if (("A" <= str && str <= "Z")) {
      ...
      for (index = j = 0, len = Literals.length; j < len; index = ++j) {
        word = Literals[index];
        if (word[0] === str) {
          Convol.push([3, index, true]);
          flag = true;
        }
      }
      if (flag === false) {
        Literals.push([str, false, false, false, false, false]);
        Convol.push([3, Literals.length - 1, true]);
      }
      return true;
    }
    return false;
